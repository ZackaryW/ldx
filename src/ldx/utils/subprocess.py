...
def query(path: str, *args, timeout: int = None, strip: bool = False, encoding: str = "gbk"):
    """
    ...
    Args:
        ...
        *args: Additional arguments to pass to the executable.
        timeout (int, optional): The maximum time in seconds to wait for the subprocess to complete. If not provided, the `DEFAULT_QUERY_TIMEOUT` value will be used.
        strip (bool, optional): Whether to strip whitespace from the output. Defaults to False.
        encoding (str, optional): The encoding to use when decoding the output. Defaults to "utf-8". Use "gbk" for Chinese Windows output.
    ...
    return raw.decode(encoding).strip() if strip else raw.decode(encoding)
